...
public class MessageHandlerTest {
  ...
  private Message connectionEndMessage() {
    return new MessageBuilder().withHeader(Header.CONNECTION).withStatus(Status.END)
               .withStatement("End of a message").build();
  }
  ...
  private MessageHandler produceMessageHandler(EventTrigger eventTrigger) {
    ...
    MessageHandler messageHandler = new MessageHandler(triggers, reporter);
